...
from app.schema import DiabetesRiskInput, RiskResponse
...
def get_top_factors(row: pd.DataFrame, n: int = 3) -> list:
    ...
    try:
        ...
        values = np.array(raw[1] if isinstance(raw, list) else raw).flatten()
...
async def favicon():
    icon_path = BASE_DIR / "templates" / "favicon.ico"
